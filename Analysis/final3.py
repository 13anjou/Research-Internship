...
def lecture_noms() : #On regarde l'ensemble des noms possibles et on selectionne les bons
	...
	with open('C:\\Users\\valentin\Desktop\\boulot_mines\\S3Recherche\\BDD\\donneesBrutes\\miTags_Bacteria\\miTAG.SILVA.RDP.relabund.txt', 'r') as csvfile:
...
def analyse_data (log_abondances, tailles, nod,k,rang,sN,dictionnaire) :
	...
	for  nom in nod :
		...
		if log_abondances[indice]!=[] and log_abondances[indice]!=[0]:
			...
			if tR>0 and solution != []:
				with open('C:\\Users\\valentin\Desktop\\boulot_mines\\S3Recherche\\BDD\\ProtistTV9subtabs293\\analyse\\{}_st{}.csv'.format(k,nom), 'wb') as csvfile:
					writer = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
					for dot in solution :
