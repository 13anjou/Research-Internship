...
def analyse_data (log_abondances, tailles, nod,k,rang,sN,dictionnaire) :
	...
	for  nom in nod :
		...
		with open('C:\\Users\\valentin\Desktop\\boulot_mines\\S3Recherche\\BDD\\ProtistTV9subtabs293\\analyse\\{}_st{}_{}_{}.csv'.format(k,sN1,sN1,nom), 'wb') as csvfile:
			writer = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
			for dot in solution :
...
def lancement(k) : 
	...
	with open('C:\\Users\\valentin\Desktop\\boulot_mines\\S3Recherche\\BDD\\ProtistTV9subtabs293\\analyse\\ecartPowerLaw_{}.csv'.format(k), 'wb') as csvfile:
			...
			for diff in abondanceMoyenne :
				print(stationNoms[indice])
				writer.writerow("{0};{1}".format(stationNoms[indice],diff))
				indice = indice + 1
